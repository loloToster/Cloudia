...
function ItemList() {
    ...
    return (
        <div className="items">
            ...
            {!loading && (
                <div className="items__quick-actions">
            ...
            {!loading && items.map(item => {
                const icon = item.icon ? `/icons/${item.icon}.png` : `/cdn/${item.id}`

                return (
                    <div onClick={() => handleFileClick(item.id)}
                        style={{ "--src": `url("${icon}")` } as React.CSSProperties}
                        className={`items__file ${item.icon ? "items__file--with-icon" : ""}`}
                        key={item.id}>
                        <div className="items__options">
                            <div className="items__metadata">
                                <div className="items__title">{item.title}</div>
                                <div className="items__user">{item.ip}</div>
                            </div>
                            <a onClick={e => e.stopPropagation()} href={`/cdn/${item.id}`} download={item.id} className="items__button items__button--download">
                            ...
                            <button onClick={e => handleDelete(e, item.id)} className="items__button items__button--delete">
                                <svg xmlns="http://www.w3.org/2000/svg" height="24" width="24">
